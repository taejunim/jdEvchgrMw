...
function get_data() {
 	...
 	$.ajax({
		...
		if(data == null || data == "")
			...
			dataHtml += "<tr id='${(status.count)}' ><input type='hidden' value = '${resultList.msgDataSeq}'><th scope='row'>${paginationInfo.totalRecordCount - 10 * (chgrInfoVO.pageIndex-1)+index}</th><td class='txt_center'>${resultList.msgSendType}</td><td class='txt_center'>${resultList.msgActionType}</td>";
			dataHtml += "<td class='txt_center'>${resultList.msgData}</td><td class='txt_center'>${resultList.regDt}</td></tr>";
